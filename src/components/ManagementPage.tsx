...
function QuickCommandsManagement() {
  ...
  const handleAdd = async () => {
    if (!newDesc.trim() || !newCmd.trim()) {
      toast(t("management.quickCommandsFillBoth"), { variant: "warning" });
      return;
    }
    setSaving(true);
    try {
      ...
      setNewDesc("");
      setNewCmd("");
      toast(t("management.quickCommandsAdded"), { variant: "success" });
    } catch (err) {
      toast(t("management.quickCommandsAddFailed"), { description: String(err), variant: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (desc: string) => {
    try {
      ...
      toast(t("management.quickCommandsDeleted"), { variant: "success" });
    } catch (err) {
      toast(t("management.quickCommandsDeleteFailed"), { description: String(err), variant: "error" });
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setEditingDesc(null);
      setEditCmd("");
      toast(t("management.quickCommandsUpdated"), { variant: "success" });
    } catch (err) {
      toast(t("management.quickCommandsUpdateFailed"), { description: String(err), variant: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ConfigLayout className="mgmt-section">
      <div className="mgmt-section-header">
        <h3 className="mgmt-section-title">{t("management.quickCommands")}</h3>
        <span className="mgmt-section-count">{t("management.quickCommandsCount", { n: entries.length })}</span>
      </div>

      <div className="mgmt-add-row">
        <input
          className="input"
          placeholder={t("management.quickCommandsDesc")}
          value={newDesc}
        ...
        <textarea
          ...
          placeholder={t("management.quickCommandsCmd")}
          value={newCmd}
        ...
        >
          {t("management.quickCommandsAdd")}
        </button>
      </div>

      {entries.length === 0 && (
        <div className="mgmt-empty">
          <p>{t("management.quickCommandsEmpty")}</p>
        </div>
      )}
              ...
              {editingDesc === desc ? (
                ...
                <>
                  <button
                    ...
                    title={t("management.quickCommandsEdit")}
                  >
                  ...
                  <button
                    ...
                    title={t("management.quickCommandsDelete")}
                  >
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
function BlacklistManagement() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewItem("");
      toast(t("management.blacklistAdded"), { variant: "success" });
    } catch (err) {
      toast(t("management.blacklistAddFailed"), { description: String(err), variant: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ConfigLayout className="mgmt-section">
      <div className="mgmt-section-header">
        <h3 className="mgmt-section-title">{t("management.blacklist")}</h3>
        <span className="mgmt-section-count">{t("management.blacklistCount", { n: items.length })}</span>
      </div>

      <p className="mgmt-section-desc">
        {t("management.blacklistDesc")}
      </p>
      ...
      <div className="mgmt-add-row">
        ...
        >
          {t("management.blacklistBlock")}
        </button>
      </div>

      {items.length === 0 && (
        <div className="mgmt-empty">
          <p>{t("management.blacklistEmpty")}</p>
        </div>
      )}
...
export function ManagementPage() {
  ...
  const tabs: { key: "commands" | "blacklist" | "settings"; label: string }[] = [
    { key: "commands", label: t("management.quickCommands") },
    { key: "blacklist", label: t("management.blacklist") },
    { key: "settings", label: t("management.settings") },
  ];

  return (
    <div className="mgmt-page">
      <div className="mgmt-page-header">
        <h2 className="mgmt-page-title">{t("management.title")}</h2>
        <div className="mgmt-page-tabs">
          {tabs.map((tabItem) => (
            <button
              key={tabItem.key}
              className={`mgmt-page-tab ${managementTab === tabItem.key ? "mgmt-page-tab--active" : ""}`}
              onClick={() => setManagementTab(tabItem.key)}
            >
              {tabItem.label}
            </button>
          ))}
